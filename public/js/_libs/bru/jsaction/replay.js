...
goog.provide('bru.jsaction.replay');
...
/**
 * Replays an event.
 * @param {!bru.jsaction.ReplayInfo} replayInfo The replay info record.
 */
bru.jsaction.replay.replayEvent = function(replayInfo) {
  var event = bru.jsaction.replay.createEvent_(replayInfo.event);
  ...
  event[bru.jsaction.EventContract.PROPERTY_KEY_REPLAY_INFO] = replayInfo;

  bru.jsaction.replay.triggerEvent_(replayInfo.element, event);
};
 ...
 * @private
 */
bru.jsaction.replay.createEvent_ = function(original) {
  var event;
 ...
 * @private
 */
bru.jsaction.replay.triggerEvent_ = function(elem, event) {
