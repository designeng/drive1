...
goog.provide('bru.jsaction.Dispatcher');
goog.provide('bru.jsaction.HandlerFunction');
goog.provide('bru.jsaction.LoaderFunction');

goog.require('bru.jsaction.replay');
 ...
 * @constructor
 */
bru.jsaction.Dispatcher = function() {
  /**
   * The handler registry.
   * @type {!Object.<string, !bru.jsaction.HandlerFunction>}
   * @private
   */
  this.handlers_ = {};
  ...
  /**
   * The loader registry.
   * @type {!Object.<string, !bru.jsaction.LoaderFunction>}
   * @private
   */
  this.loaders_ = {};
};
 ...
 * @private
 */
bru.jsaction.Dispatcher.ATTRIBUTE_NAME_JSNAMESPACE_ = 'data-namespace';
 ...
 * @private
 */
bru.jsaction.Dispatcher.prototype.queue_;
 ...
 *     to attach to.
 */
bru.jsaction.Dispatcher.prototype.attach = function(contract) {
 ...
 * @param {!bru.jsaction.LoaderFunction} loaderFn The loader function.
 */
bru.jsaction.Dispatcher.prototype.registerLoader = function(ns, loaderFn) {
 ...
 * @param {string} ns The namespace.
 * @param {!Object.<string, !bru.jsaction.HandlerFunction>} handlers
 *     The handlers. Map from action name to action handler function.
 * @param {Object=} opt_scope
 */
bru.jsaction.Dispatcher.prototype.registerHandlers = function(ns, handlers, opt_scope) {
  for (var name in handlers) {
    var action = ns + '.' + name;
    bru.jsaction.Dispatcher.assertValidAction_(action);
    this.handlers_[action] = opt_scope ?
        goog.bind(handlers[name], opt_scope) : handlers[name];
  }
 ...
 */
bru.jsaction.Dispatcher.prototype.dispatch = function(
    action, elem, e, time) {
  ...
  if (action.indexOf('.') == -1) {
    for (var ancestor = elem; ancestor; ancestor = ancestor.parentNode) {
      ns = ancestor.getAttribute(
          bru.jsaction.Dispatcher.ATTRIBUTE_NAME_JSNAMESPACE_);
      if (ns) {
        action = ns + '.' + action;
        break;
      }
  ...
  ns = bru.jsaction.Dispatcher.getNamespace_(action);
 ...
 * @private
 */
bru.jsaction.Dispatcher.prototype.maybeInvokeHandler_ = function(
    action, elem, e, time) {
  var handler = this.handlers_[action];
  if (handler) {
    handler(new goog.jsaction.Context(action, elem, e, time));
    // Возможно тут стоит придумать более продвинутую проверку чтобы
    // при попытке отправить _другую_ форму выдавалось предупреждение.
    if (e.type == 'submit') {
      window.onbeforeunload = null;
    } else if (elem.nodeName == 'A' && elem.getAttribute('data-npd') === null) {
 ...
 * @private
 */
bru.jsaction.Dispatcher.getNamespace_ = function(action) {
 ...
 * @private
 */
bru.jsaction.Dispatcher.assertValidAction_ = function(action) {
 ...
 * @private
 */
bru.jsaction.Dispatcher.prototype.hasQueuedActionInNamespace_ = function(ns) {
  if (this.queue_) {
    for (var i = 0; i < this.queue_.length; ++i) {
      if (ns == bru.jsaction.Dispatcher.getNamespace_(this.queue_[i].action)) {
        return true;
      }
    }
  }
 ...
 * @private
 */
bru.jsaction.Dispatcher.prototype.replayEvents_ = function() {
  if (!this.queue_) {
    return;
  }
  for (var i = 0, replayInfo; replayInfo = this.queue_[i]; ) {
    if (replayInfo.action in this.handlers_) {
      ...
      bru.jsaction.replay.replayEvent(replayInfo);
    } else {
      ++i;
    }
  }
